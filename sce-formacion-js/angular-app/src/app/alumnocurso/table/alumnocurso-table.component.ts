...
@Component({
  selector: 'app-alumnocurso-table',
  templateUrl: './alumnocurso-table.component.html'
})
export class AlumnoCursoTableComponent implements OnInit, OnDestroy {
  ...
  search(pageable?: Pageable) {
    ...
    const search$ = this.alumnoCursoService.query(mergedFilters, pageable);

    search$.subscribe(
      alumnosCursos => {
        this.totalRecords = alumnosCursos.totalRecords;
        this.tableValue = alumnosCursos.data;
  ...
  private loadSearchStatus() {
    const storedStatus = this.searchService.getStoredStatus('alumnosCursos');
    if (storedStatus) {
  ...
  private storeSearchStatus() {
    this.searchService.storeStatus('alumnosCursos', {
