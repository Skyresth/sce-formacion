...
export class CursoTableComponent implements OnInit, OnDestroy {
  ...
  cols = [
    ...
    { field: 'fechaInicio', header: 'Fecha inicio', date: true },
    { field: 'fechaFin', header: 'Fecha fin', date: true },
  ];
  ...
  search(pageable?: Pageable) {
    ...
    const search$ = this.primaryButtonAction ?
      this.cursoService.searchLinks(mergedFilters, pageable) :
      this.cursoService.query(mergedFilters, pageable);

    search$.subscribe(
      cursos => {
        this.totalRecords = cursos.totalRecords;
        this.tableValue = cursos.data;
  ...
  private loadSearchStatus() {
    const storedStatus = this.searchService.getStoredStatus('cursos');
    if (storedStatus) {
  ...
  private storeSearchStatus() {
    this.searchService.storeStatus('cursos', {
